 ...
 */
const findNextAvailableCounter = async (baseName: string, bucket: string): Promise<string> => {
...
export const uploadToSpaces = async (filePath: string, channelName: string): Promise<string> => {
  try {
    ...
    const uniqueFileName = await findNextAvailableCounter(sanitizedChannelName, config.DO_SPACES_BUCKET);
